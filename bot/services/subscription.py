"""Проверка подписки на канал бренда для раздела «Астропрогноз 2027».

Подписка проверяется при каждом входе в раздел, но результат кешируется
в памяти на час по user_id, чтобы не дёргать Telegram API на каждое
нажатие.

Если бот не админ в канале, getChatMember вернёт ошибку. В этом случае
раздел открывается без проверки — ломаться он не должен, — а в лог
уходит warning (ТЗ, раздел 9).
"""
